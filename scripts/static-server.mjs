...
const root = normalize(join(repositoryRoot, process.argv[2] || '.'));
...
const server = createServer((request, response) => {
  ...
  if (!file.startsWith(root) || !statSync(file, { throwIfNoEntry: false } )?.isFile()) {
    ...
    if (!file.startsWith(root) || !statSync(fallback, { throwIfNoEntry: false })?.isFile()) {
      response.writeHead(404); response.end('Not found'); return;
    }
    response.writeHead(200, { 'Content-Type': types['.html'] });
    createReadStream(fallback).pipe(response); return;
  }
